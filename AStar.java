...
public class AStar {
    ...
    void calculatepath(int[][] matrix) {
       ...
       for(int k = 0; k < frontier.size(); k++ ){
            visitedFlag = false;
            for (Node expansionList1 : expansionList) {
                if ((frontier.get(k).getI() == expansionList1.getI() && frontier.get(k).getJ() == expansionList1.getJ()) && frontier.get(k).getNodeType() != NodeType.StartNode) {
                    visitedFlag = true;
                }
            }
            if(visitedFlag) {
                continue;
                 ...
                 int i = frontier.get(k).getI();
                 int j = frontier.get(k).getJ();
                  ...
                  if(frontier.get(k).getNodeType() != NodeType.StartNode) {
